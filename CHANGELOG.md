...
1.30.1
- Fix: if the OTP server is not accessible, the request response throws an exception due to the response type (boolean vs expected array)
...
- Add a setting to set the periodicity of the background job that checks for completed signature requests
...
- Add a meaningful error message when trying to perform an advanced signature on something else than a PDF file
